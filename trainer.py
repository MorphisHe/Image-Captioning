...
class Trainer:
    ...
    def train_one_step(self, images, captions, lengths):
        ...
        loss = self.criterion(
            torch.flatten(outputs, start_dim=0, end_dim=1), # (batch_size*padded_seq_length, vocab_size)
            torch.flatten(captions, start_dim=0, end_dim=1) # (batch_size*padded_seq_length)
        )
        loss.backward()
        self.optimizer.step()
        ...
        with torch.no_grad():
            ...
            for images, captions, lengths in self.dev_dataloader:
                ...
                for caption in captions:
                    ...
                    end_seq_idx = encoded_caption.index(vocab.word2idx[END_SEQ])
                    encoded_caption = encoded_caption[:end_seq_idx]
            ...
            for generated_seq in generated_seqs:
                ...
                if end_seq_val in generated_seqs:
